function [Matrix,count,NR,TM]=read_Curry_file3(filename,type,needContinue,Continued)
...
% Output: Matrix is the matrix of data read from the file
%         count is the number of the data read from the file
%         NR equals number of rows
%         TM is transformation matrix between locations and curry internal coordinate system
if ~Continued
    begin=0;
    matched=0;
    found=0;
...
while (~feof(fid) & ~found)
    ...
    %elseif (~isempty( findstr(['POINT_TRAFO END_LIST'],TLine)) )
        %end_header=1;
    elseif (~isempty( findstr(['POINT_TRAFO START_LIST'],TLine)) )
        [TM,count]=fscanf(fid,'%f',[4,4]); 
        TM = TM';
