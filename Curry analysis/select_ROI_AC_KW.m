...
function [ROI_ind,ROI_ind_group,ROI_ind_group2,Rx,allmasks]=select_ROI_AC_KW(varargin) 
...
% deg_alpha = 0;
% remakemask = 1;
%[ROI_ind,ROI_ind_group,ROI_ind_group2,Rx,allmasks]=select_ROI_AC('WB',-30,-5,1);
...
%     createmaskflag=0;
    ...
    %cdr_file=['rbic_4'];
    cdr_file=['sLORETA_Open'];
%     cdr_BL_file=['RE_new_BL'];
    
    disp('********************');
    disp(cdr_file)
    cdr_file_name=[Base_dir,subjectName,'\new\',cdr_file,'.cdr'];
%     cdr_file_name=[Base_dir,subjectName,'\AKpost\New-result-using-CC-final\',cdr_file,'.cdr'];
%     cdr_BL_file_name=[Base_dir,subjectName,'\new\',cdr_BL_file,'.cdr'];
    
    %list of subjects using Albert's pts
    subjectNames_allregion = ['JR']; %['CM110206','JS4new','CM1new2','FNrobot1', 'FNrobot2', 'YZrobot1', 'LS1', 'LS2','LSpost1','LSpost2','GMrobot','JS2','CM103006','JS2new','JS3','AKpost1','AKpost2','AKpre1','AKpre2','WBrobot','JRrobot1','JRrobot2','JRrobot3','HCrobot','EWrobot1','EWrobot2','BJrobot1','BJrobot2','NRrobot1','NRrobot2','HOrobot1','HOrobot2','DHrobot1','DHrobot2','DHrobot3','KCrobot','EBrobot1','EBrobot2','JCrobot','KCrobot','JBrobot1'];
    subjectNames_newregion = [];%['WSrobot', 'WSrobot2', 'WSrobot3','JWrobot','JWrobot2','NTrobot1','NTrobot2','JDrobot','BSrobot1'];
    %subjectNames_newregion = [];
    
    %EDGE IS USED IN THIS FILE TO MEAN "REGION"
    if strfind(subjectNames_allregion,subjectName)
        %using Curry 5.0 Albert's pts
        edge1_file_name=[Base_dir,subjectName,'\Points\','allregions_ac_new.pom'];%all regions
%         edge1_file_name=[Base_dir,subjectName,'\Points\','new_region.pom'];%all regions
...
%         censul_file_name1=[Base_dir,subjectName,'\Points\','CS_ac.pom'];   %central sulcus
        ...
        edge1_file_name=[Base_dir,subjectName,'\Points\','R-M1-KW.pom'];%M1_rt
        edge2_file_name=[Base_dir,subjectName,'\Points\','L-M1-KW.pom'];%M1_lt
        edge3_file_name=[Base_dir,subjectName,'\Points\','R-S1-KW.pom'];%S1_rt
        edge4_file_name=[Base_dir,subjectName,'\Points\','L-S1-KW.pom'];%S1_lt
        edge5_file_name=[Base_dir,subjectName,'\Points\','R-PM-KW.pom'];%PM_rt
        edge6_file_name=[Base_dir,subjectName,'\Points\','L-PM-KW.pom'];%PM_lt
        edge7_file_name=[Base_dir,subjectName,'\Points\','R-SMA-KW.pom'];%SMA_rt
        edge8_file_name=[Base_dir,subjectName,'\Points\','L-SMA-KW.pom'];%SMA_lt
        ...
        if exist (edge1_file_name) 
            [edge1,Ecount,ENR]=read_Curry_file4_AC(edge1_file_name,'LOCATION',0,0);
        else
            edge1 =[];
        end
        if exist (edge2_file_name) 
            [edge2,Ecount,ENR]=read_Curry_file4_AC(edge2_file_name,'LOCATION',0,0);
        else
            edge2 =[];
        end
        if exist (edge3_file_name) 
            [edge3,Ecount,ENR]=read_Curry_file4_AC(edge3_file_name,'LOCATION',0,0);
        else
            edge3 =[];
        end
        if exist (edge4_file_name)      
            [edge4,Ecount,ENR]=read_Curry_file4_AC(edge4_file_name,'LOCATION',0,0);
        else
            edge4 =[];
        end
        if exist (edge5_file_name)
            [edge5,Ecount,ENR]=read_Curry_file4_AC(edge5_file_name,'LOCATION',0,0);
        else
            edge5=[];
        end
        if exist (edge6_file_name)
            [edge6,Ecount,ENR]=read_Curry_file4_AC(edge6_file_name,'LOCATION',0,0);
        else
            edge6=[];
        end
        if exist (edge7_file_name)
            [edge7,Ecount,LNR,TM7]=read_Curry_file3_TM(edge7_file_name,'LOCATION',0,0);%[edge7,Ecount,ENR]=read_Curry_file4_AC(edge7_file_name,'LOCATION',0,0);
        else
            edge7=[];
        end
        if exist (edge8_file_name)        
        ...
        right_edge = [edge1;edge3;edge5;edge7];
        left_edge = [edge2;edge4;edge6;edge8];
...
%     h3 = imagesc(imA_resize3);
    ...
    %----------------------------------------------------------
    %User picks mask
    fprintf(1, '\nPlease press 1, 2, 3, 4, 5, 6, 7, 8 in turn to select RM1, LM1, RS1, LS1, RPM, LPM, RSMA, LSMA. When it is all done, please press enter!\n')
    fprintf(1, '\nTips: You can escape 1 and 2 (RM1 & LM1), since their index is 1 and 2 anyway.')
    but = 1;
    mask_value = 1;
    while (but >= 1 & but <=3) | (but >= 49 & but <=56)
        [xi,yi,but] = ginput(1);
        if (but >= 1 & but <=3) | (but >= 49 & but <=56)
...
%     mask_perim_nums = union(mask_perim_nums,[areanum]);
    ...
    [mask_m,mask_n] = find(mod(bigmask4,2) == 0 & bigmask4>0);
    for mask_ind = 1:length(mask_m)
    ...
    %odd numbers
    [mask_m,mask_n] = find(mod(bigmask4,2) == 1);
    for mask_ind = 1:length(mask_m)
...
%     top_cortex_ind=indzs;
    ...
    for i=1:n
        ...
        if min(dis2total_edge)<10 % if miminum distance to the cc points is less than 10, then count it; otherwise, delete it
