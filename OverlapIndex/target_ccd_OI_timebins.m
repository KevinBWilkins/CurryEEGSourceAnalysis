...
function [OI,ActiveRatio,OverArea_r,OAA, OverArea_r1,OAA1, maxStrenght, CoG]=target_ccd_OI_timebins(subject,group)
...
% 			[cortexL,targetL,targetC]=find_target_PC(cdr_file_name,edge1_file_name,edge2_file_name,timePoints,plotCort);
        [cortexL,targetL,targetC_wh_time(i,:,:)]=find_target_PC4_old(cdr_file_name,mask_file_name,timePoints,plotCort);
%         load targetC_wh_time
    ...
    for phTime=1:LoopNo+1
        ...
        for i=1:length(taskList)
            cur_task=taskList{i};
            targetC=squeeze(targetC_wh_time(i,:,cur_window));
