...
function target_ccd_cog_fig(method)
...
% 			cdr_file_name=[Base_dir,subjectName,'\results_inv\Jules_',cdr_file];			
...
%                     plot3(censul(:,1),censul(:,2),censul(:,3),'LineWidth',7,'color','g','LineStyle','.');
                    plot3(censul(:,1),censul(:,2),censul(:,3),'Makersize',4,'color','g','Maker','.');
%                     plot3(center_now(:,1),center_now(:,2),center_now(:,3),'m*')
                    
                    view(90,90)
                    title([cur_task,'    Time=',num2str(-100+timenow*3.9),'(ms)'],'FontSize',8); 
	%                 pause
	%                 clf
                end
            end
