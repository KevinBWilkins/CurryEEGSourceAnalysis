function [fig_h]=plot_data_on_cortex(subjectName, deg_alpha, Base_dir, targetL, censul, targetC, cortexL, index, taskColorMap, firstFlag, cur_window)
...
%         end
    ...
    %             cur_window = (phTime-1)*(winSize-winOverlap)+1+startPoints:(phTime-1)*(winSize-winOverlap)+winSize+startPoints;
                %8 or 16 subplots
        %         subplot(4,4,index)
            subplot(4,6,firstFlag+2*(index-1))
...
%                 C_bin(Thr_ind) = 0;
        ...
        if firstFlag == 1
            surface('XData',rot_XI,'YData',rot_YI,'ZData',rot_ZI,'CData',CI_bin,'FaceColor','interp','EdgeColor','none','FaceAlpha',0.6);
        elseif firstFlag == 2
            surface('XData',rot_XI,'YData',rot_YI,'ZData',rot_ZI,'CData',CI_bin,'FaceColor','interp','EdgeColor','none','FaceAlpha',0.6);
            
%             contour(rot_XI, rot_YI, CI_bin);
        elseif firstFlag == 3
            plot3(rot_targetL(:,1),rot_targetL(:,2),rot_targetL(:,3),'m.','MarkerSize',1);
        end
...
%         end
            view(0,90)
            TR = cur_window;
            title([num2str((TR(1)-1)*3.991+timestart),' to ',num2str(TR(end)*3.991+timestart),' ms'],'Fontsize',8,'FontWeight','bold')
            axis off
            caxis([0 1])
            axis equal
            axis xy
