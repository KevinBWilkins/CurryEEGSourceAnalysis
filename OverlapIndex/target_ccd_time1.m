...
function [OI,AOI,max_CI]=target_ccd_time(subject)
...
%             Tes = delaunay3(targetL(:,1),targetL(:,2),targetL(:,3));
            ...
            for timenow=1:timePoints
                ...
                CI_norm=griddata(targetL(:,1),targetL(:,2),C,XI,YI);
				TA=isnan(CI);
				ind=find(TA==1);
				CI(ind)=0;
                CI_norm(ind)=0;
...
%                     view(-90,90)
                ...
                if timenow==1
                    pre_current=CI;
                    pre_norm_current=CI_norm;
                else
                    cur_current=CI;
                    cur_norm_current=CI_norm;
                    OI(i,timenow-1)=sum(sum(pre_current.*cur_current,1),2)/sum(sum(pre_current,1),2);
                    AOI(i,timenow-1)=sum(sum(pre_norm_current.*cur_norm_current,1),2)/size(CI_norm,1)/size(CI_norm,2);
                    max_CI(i,timenow-1)=max(targetC(:,timenow));
                    cur_current=pre_current;
                    cur_norm_current=pre_norm_current;
                end
            end% end of timenow
