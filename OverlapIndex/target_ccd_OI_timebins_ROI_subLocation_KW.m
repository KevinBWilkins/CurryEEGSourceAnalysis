...
function [act_ratio_loc_time]=target_ccd_OI_timebins_ROI_subLocation_KW(subject,arm)
...
taskList={'sLORETA_LiftOpen'}; %Post-Open-Long
phaseList={'whole'};
...
%  winOverlap=1; % These are the values for my (KW) long segments
%  winSize=13;
%  timePoints=205;

winOverlap=1; 
winSize=13;
timePoints=14; %was 14 or 205
...
startPoints=timePoints-needed_len;
...
for k=1:subNo
    ...
    elseif subjectArm == 'r'
        %location_order = [6 8 2 4 3 1 7 5];  %Jun's order which is not
        %consistent
        location_order = [4 2 8 6 5 7 1 3]; %my order: cS1,cM1,cSMA,cPM,iPM,iSMA,iM1,iS1
    end
...
% 			[cortexL,targetL,targetC]=find_target_PC(cdr_file_name,edge1_file_name,edge2_file_name,timePoints,plotCort);
        [cortexL,targetL_all,targetC_wh_time_ROI(:,i)]=find_target_PC4_KW(cdr_file_name,mask_file_name,timePoints,plotCort,1:8);
...
%                 end
                  %act_ratio_loc_time (k,i,i_location,phTime)= sum(sum(targetC)); 
                  ...
                  %act_ratio_loc_time (k,i,i_location,phTime)= sum(sum(targetC))/(3172.7); %user (i.e.KW) specified max value for the 2 conditions
